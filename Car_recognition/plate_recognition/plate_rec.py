...
def get_plate_result(img, device, model):
    ...
    # 将预测结果转换为 numpy 数组
    preds = preds.view(-1).detach().cpu().numpy()
    color_preds = color_preds.argmax(dim=-1).item()

    # 解码预测结果
    newPreds = decodePlate(preds)
    plate = "".join([plateName[i] for i in newPreds])

    return plate, color_list[color_preds], min_confidence
